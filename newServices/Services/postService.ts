import { POSTS,POSTS2, REQUESTURL, USERS, NEWS, NEWS2 } from "../constants/urls";
...
export async function fetchPosts() {
    try {
        const respone = await fetch(POSTS2);
...
export async function fetchPost(postId:number){
    const res = await fetch(`${POSTS2}/${postId}`);
...
export async function updatePost_PUT(incrementer: number, postId: number) {
    ...
    const res_2 = await fetch(`${POSTS2}/update`, {
        method: 'POST',
        headers: {
